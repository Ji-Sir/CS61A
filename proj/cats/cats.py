...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def aboutWord(p):
        # split(remove_punctustion(lower(p)))
        for j in range(len(split(remove_punctuation(lower(p))))):
            for k in range(len(subject)):
                if(subject[k] == split(remove_punctuation(lower(p)))[j]):
                    return True
        return False
    return aboutWord
    # END PROBLEM 2
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if(typed_words[i] == source_words[i]):
            num += 1
    if(num == 0 and max(len(typed_words), len(source_words)) == 0):
        return 100.0
    if(num == 0 and max(len(typed_words), len(source_words)) != 0):
        return 0.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff_values = [diff_function(typed_word, word, limit) for word in word_list]
    min_diff_index = min(range(len(diff_values)), key = lambda i: diff_values[i])
    if diff_values[min_diff_index] <= limit:
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[timestamps_per_player[j][k] - timestamps_per_player[j][k - 1] for k in range(1, len(timestamps_per_player[j]))] for j in range(len(timestamps_per_player))]
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_all_times(match)[j][i] < min_time:
                ...
                fastest_player = j
        word[fastest_player].append(get_all_words(match)[i])
    return word
    # END PROBLEM 10
